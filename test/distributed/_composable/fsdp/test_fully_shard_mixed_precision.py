...
class TestFullyShardMixedPrecisionTraining(FSDPTest):
    ...
    def _test_grad_dtype_preserved_grad_acc(
        ...
        for microbatch_idx in range(num_microbatches):
            ...
            if not is_last_microbatch:
                self._assert_accumulate_dtype(model, grad_dtype, reduce_dtype)

        for param in model.parameters():
            self.assertIsNotNone(param.grad)
            self.assertEqual(param.grad.dtype, grad_dtype)

    def _assert_accumulate_dtype(self, model, grad_dtype, reduce_dtype):
        # Microbatch sums must happen at the wider of the two. Checking only the
        # final `param.grad.dtype` would miss summing narrow and casting after.
        expected = max((reduce_dtype, grad_dtype), key=lambda dtype: dtype.itemsize)
        for module in model.modules():
            ...
            for fsdp_param in state._fsdp_param_group.fsdp_params:
                for grad in (
                    ...
                    if grad is not None:
                        self.assertEqual(grad.dtype, expected)
        ...
        self.run_subtests(
            {
                ...
                # bfloat16 covers reduce_dtype narrower than grad_dtype
    ...
    @skip_if_lt_x_gpu(2)
    def test_grad_dtype_none_keeps_orig_dtype(self):
        """grad_dtype=None means any gradient dtype is allowed, not "skip the
        cast back to orig_dtype". Set after fully_shard, since reading it off
        the live parameter is what made the cast target None."""
        torch.manual_seed(42)
        model = nn.Sequential(*[MLP(16, torch.device("cpu")) for _ in range(3)])
        model.to(device=device_type, dtype=torch.float32)

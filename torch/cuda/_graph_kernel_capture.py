...
# Modules announced with no image, or with one that does not look like an ELF.
# Counted rather than warned about on the spot: this runs inside CUPTI's C
# dispatch, where warnings.warn could be configured to raise.
...
def stop() -> None:
    ...
    if _skipped_modules:
        logger.warning(
            "skipped %d module load(s) with no usable cubin image; kernels from "
            "those modules will not be serializable",
            _skipped_modules,
        )
        _skipped_modules = 0

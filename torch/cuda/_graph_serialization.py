...
def _kernel_args(driver: Any, params: Any) -> tuple[list[bytes], bytes | None]:
    ...
    be handled; the bytes are copied out verbatim either way.
    """
...
def _collect_nodes(
    ...
    """Serialize every node's parameters, the topology connecting them, and how many
    distinct events the event nodes refer to."""
    ...
    *_, edge_count = _chk(driver.cuGraphGetEdges(raw_graph, 0))
    if edge_count:
        src, dst, *_ = _chk(driver.cuGraphGetEdges(raw_graph, edge_count))
        edges = [[index[int(u)], index[int(v)]] for u, v in zip(src, dst)]
    else:
        edges = []
...
def _select_cubins(driver: Any, names: set[str]) -> dict[str, int]:
    ...
    if missing:
        raise UnserializableGraph(
            ...
            f"in another process: {missing}. Capture was probably armed after they "
            "were loaded."
        )
    return resolved
...
def _check_no_cudamalloc_anywhere() -> None:
    """Refuse if anything in the process was allocated with cudaMalloc.

    Not just the graph's own pools: a cudaMalloc address cannot be reclaimed at a
    fixed virtual address at all, so its presence means the process is not in the
    all-VMM regime and something reachable from the graph may be unrestorable.
    """
    import torch
    ...
    for segment in torch.cuda.memory_snapshot(include_traces=False):
        if not segment["is_expandable"]:
            raise UnserializableGraph(
                f"the caching allocator holds a non-expandable segment at "
                f"{segment['address']:#x}. Saving requires the whole process to use "
                "expandable segments (PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True) "
                "and to avoid allocators that bypass them, such as legacy cuBLAS "
                "(cuBLASLt is fine)."
            )
...
def save(
    ...
    *,
    ...
    _check_no_cudamalloc_anywhere()
    nodes, edges, num_events = _collect_nodes(driver, raw_graph)
    ...
    # Anything still unaccounted for was not allocated by the caching allocator,
    # so no snapshot segment covers it and it cannot be brought back.
...
def _topological_order(count: int, edges: list[list[int]]) -> list[int]:
    ...
    cuGraphGetNodes does not promise an order, so the saved order cannot be trusted
    to have parents first -- and cuGraphAddKernelNode needs its dependencies to
    exist already.
    """
    ...
    **CPU**. The archive carries no data (parameters evolve after capture, so bytes
    ...
    for index in _topological_order(len(manifest["nodes"]), manifest["edges"]):
        ...
        if kind == "kernel":
            ...
            handle = _chk(driver.cuGraphAddKernelNode(graph, deps, len(deps), params))
            ...
            handle = _chk(
                driver.cuGraphAddMemcpyNode(graph, deps, len(deps), copy, context)
            )
        elif kind == "memset":
            fields = node["params"]
            ...
            context = _chk(driver.cuCtxGetCurrent())
            handle = _chk(
                driver.cuGraphAddMemsetNode(graph, deps, len(deps), fill, context)
            )
        elif kind == "event_record":
            handle = _chk(
                driver.cuGraphAddEventRecordNode(
                    graph, deps, len(deps), events[node["event"]]
                )
            )
        elif kind == "event_wait":
            handle = _chk(
                driver.cuGraphAddEventWaitNode(
                    graph, deps, len(deps), events[node["event"]]
                )
            )
        else:
            handle = _chk(driver.cuGraphAddEmptyNode(graph, deps, len(deps)))
        made[index] = handle
